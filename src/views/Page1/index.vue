<template>
  <div class="main-container">
    <!-- 当日入库统计(0-24点) -->
    <div class="camera" @click="openModal(1)"></div>
    <div class="camera2" @click="openModal(2)"></div>
    <!-- 模态框部分 -->
...
export default {
  ...
  methods: {
    openModal(type) {
      ...
      this.videoSrc =
        type === 1
          ? require("@/assets/videos/exp.mp4")
          : require("@/assets/videos/exp2.mp4");
      // 确保模态框显示后播放视频
      this.$nextTick(() => {
        ...
        if (obj.code === "0") {
            ...
            for (let i = 0; i < obj.WMS_ORDER_STATUS.length; i++) {
              ...
              if (status.order_type == "in") {
                ...
                if (status.wms_order_state === "waiting") {
                  order_in.waiting_count += parseInt(status.count);
                } else if (status.wms_order_state === "doing") {
                  order_in.doing_count += parseInt(status.count);
                } else if (status.wms_order_state === "done") {
                  order_in.done_count += parseInt(status.count);
                } else {
                  order_in.other_count += parseInt(status.count);
                }
              } else {
                //出库单
                if (status.wms_order_state === "waiting") {
                  order_out.waiting_count += parseInt(status.count);
                } else if (status.wms_order_state === "doing") {
                  order_out.doing_count += parseInt(status.count);
                } else if (status.wms_order_state === "done") {
                  order_out.done_count += parseInt(status.count);
                } else {
                  order_in.other_count += parseInt(status.count);
                }
              }
          ...
          if (this.isValid(obj.WMS_ORDER_IN_24_HOUR)) {
            ...
            for (let i = 0; i < obj.WMS_ORDER_IN_24_HOUR.length; i++) {
              ...
              tempData.push({
                time: status.hour,
                totalAmount: parseFloat(status.total_quantity).toFixed(2),
                totalTons: parseFloat(status.total_weight).toFixed(2),
              });
            }
            for (let i = 0; i < 24; i++) {
          ...
          if (this.isValid(obj.WMS_SPACE_USED)) {
            ...
            this.storageSpace = {
              percent: parseFloat(
                (
                  (obj.WMS_SPACE_USED.volume / obj.WMS_SPACE_USED.space) *
                  100
                ).toFixed(2)
              ),
              des: "仓库空间",
          ...
          if (this.isValid(obj.WMS_QUANTITY_WEEK)) {
            ...
            // 总数据
            this.allData.oneWeekStorageData = this.oneWeekStorageData;
          }
          //#endregion
          // 员工任务状态
          //#region
          ...
          if (this.isValid(obj.WMS_ORDER_IN_WEEK)) {
            ...
            for (let i = 0; i < obj.WMS_ORDER_IN_WEEK.length; i++) {
              ...
              this.oneWeekInData.push({
                ...
                amount: parseFloat(status.total_quantity).toFixed(2),
                tons: parseFloat(status.total_weight).toFixed(2),
              });
            }
            // 总数据
